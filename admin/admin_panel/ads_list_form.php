...
<div class="span9">
	<div class="content">	
		<div class="control-group">
					<div class="controls">
						<button type="button" onclick="add_new_ad();" class="btn btn-inverse">New Ad</button>
					</div>
				</div>
		<div class="module">
			...
			<form class="form-horizontal row-fluid" id="db_entry_form" method="POST" action="">
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="text"  name="from_date" id="from_date" data_id="from_date" placeholder="Select Start Date" class="span4 datepicker" value="<?php //echo date('d-m-Y')?>">
						<input type="text"  name="to_date" id="to_date" data_id="from_date" placeholder="Select End Date" class="span4 datepicker" value="<?php //echo date('d-m-Y')?>">
...
<script>
	...
	function get_app_user_detail()
	{
		...
		var dataTable = $('#data-grid').DataTable( 
		{
			...
			"language": 
			{
				searchPlaceholder: "Description/Youtube Id"
			},
			"ajax":
			{
				url :"ads_get_list.php", // json datasource
				type: "post",  // method  , by default get
				data:{'from_date':$("#from_date").val(),'to_date':$("#to_date").val(),'status':$("#status").val(),'ad_type':$("#ad_type").val(),'interest':$("#interest").val()},
				error: function(data)
				{  
	...
	{		
		...
		data: {'type':'ads','id':get_id,'status_value':get_status_value},
		cache: false,
		dataType:'json',
	...
	function add_new_ad() 
	{
		window.location.href='ads_create_form.php';
	}
